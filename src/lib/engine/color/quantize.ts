...
/**
 * Count total pixels in a subtree (for popularity-weighted reduction).
 */
function countPixels(node: OctreeNode): number {
  let total = node.pixelCount;
  for (let i = 0; i < 8; i++) {
    if (node.children[i] !== null) {
      total += countPixels(node.children[i]!);
    }
  }
  return total;
}
...
// ---------------------------------------------------------------------------
// Popularity-weighted octree quantizer
// ---------------------------------------------------------------------------

export function octreeQuantizeWeighted(
  data: Uint8ClampedArray,
  width: number,
  height: number,
  targetColors: number,
  ...
  // ---- Step 2: Reduce ----
  // Phase A: Popularity-weighted reduction - collapse interior nodes with the
  // fewest total pixels first, preserving colors that cover large areas.
  {
    // Collect all interior nodes
    const interiorsByDepth: OctreeNode[][] = [[], [], [], [], [], [], [], []];
    ...
    for (let depth = 6; depth >= 0 && leafCount > targetColors; depth--) {
      const nodes = interiorsByDepth[depth];
      // Sort: fewest total pixels first (popularity-weighted)
      nodes.sort((a, b) => countPixels(a) - countPixels(b));
